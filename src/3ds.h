...
{
  tMaterialInfo() : rgba_texture(0), uTile(0), vTile(0), uOffset(0), vOffset(0) {strFile[0] = 0;};
  ~tMaterialInfo() {if (rgba_texture) delete rgba_texture;}
	char  strName[255];			// The texture name
	char  strFile[255];			// The texture file name (If this is set it's a texture map)
	unsigned char  color[4];				// The color of the object (R, G, B, A)
  Rgba_file_texture * rgba_texture;
...
struct t3DObject 
{
  t3DObject() : numOfVerts(0), numOfFaces(0), numTexVertex(0), materialID(0),
