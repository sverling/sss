...
{
public:
  Camera(const Position& pCenter) : Object(pCenter) {}
  virtual ~Camera() {}
  virtual void draw(Draw_type draw_type) {/* Nothing to draw */}
  virtual float get_graphical_bounding_radius() const {return 0.0f;}
  virtual float get_structural_bounding_radius() const {return 0.0f;}
  virtual bool use_physics() const {return false;}
...
#ifdef WITH_PLIB
  jsInit () ;
  m_js = new jsJoystick(m_config.plib_joystick_num);
  if (m_js->notWorking())
  {
    TRACE("PLIB Joystick %d not working/detected - use mouse/keyboard instead\n", m_config.plib_joystick_num);
  }
#endif
                     ...
                     Joystick * joystick)
{
  ...
  m_Center = body->get_pos();
  m_Camera = new Camera(m_Center);
  ...
  Gui::instance()->set_keyboard_up_func(_keyUp);
  ...
  if (m_config.use_heli_controller)
  {
    TRACE("Enabling special helicopter controller\n");
    m_glider->take_control(0); // make sure auto-pilot is off
    m_heli_controller = new Heli_controller(m_glider);
  }
  ...
  if (!m_config.running_as_screensaver)
    toggle_paused();
}
...
{
  float time_scale = m_config.time_scale;
#ifdef WIN32
  if (m_config.timer_method == Config::TIMER_PERF)
  {
    ...
    if (use_perf)
    {
      ...
      return time_scale * (float)currentTime.QuadPart / (float)frequency.QuadPart;
    }
    else
    {
      return 0.001f * time_scale * glutGet(GLUT_ELAPSED_TIME);
    }
  }
...
#else
  return (float) (0.001 * time_scale * glutGet(GLUT_ELAPSED_TIME));
#endif
}

int Sss::get_milliseconds() const
{
  float time_scale = m_config.time_scale;
#ifdef WIN32
  if (m_config.timer_method == Config::TIMER_PERF)
  {
    ...
    if (use_perf)
    {
      //      QueryPerformanceFrequency(&frequency);
      QueryPerformanceCounter(&currentTime);
      float seconds = (float)currentTime.QuadPart / (float)frequency.QuadPart;
      int milliseconds = (int) (seconds * 1000.0f);
      return time_scale * milliseconds;
    }
    else
    {
      return (int) (time_scale * glutGet(GLUT_ELAPSED_TIME));
    }
  }
  else // do not use high res timer
  {
...
void Sss::idle()
{
  ...
  if (m_config.text_overlay > Config::TEXT_NONE)
  {
    ...
    if
      (m_eye == m_Camera)
    {
      m_text_overlay->add_entry( 20, 50, "Look around");
    }
  }
  ...
  if (m_heli_controller)
    m_heli_controller->update(dt);
...
void Sss::add_robot()
{
  ...
  int offset;
  for (offset = 5 ; offset < 60 ; offset += 5)
  {
...
void Sss::display(void)
{
  ...
  if (m_show_help == true)
  {
    ...
    m_text_overlay->add_entry(x, y -= dy, "V: Toggle main variometer location and volume (EYE -> GLIDER -> SILENT)");
  }
  else
  {
    // When the main glider is set up with a Robot_pilot, it's so easy
    // to bypass it (i.e. turn the "auto-pilot" off). Much easier that
    // creating a Robot_pilot when the sim is started without
    // one. However, there is some overhead associated with the robot
    // AI, even when the end result gets ignored, though that could be
    // easily eliminated.
    if (m_config.sss_mode == Config::DEMO_MODE && m_config.text_overlay > Config::TEXT_NONE)
    {
...
void Sss::mouse(int button, int state, int x, int y)
{
  ...
  if (m_heli_controller)
    m_heli_controller->mouse(button, state, x, y);
...
void Sss::motion(int x, int y)
{
  if (m_LookAround)
  {
    int lDeltaX = x - m_StartX;
    int lDeltaY = y - m_StartY;

    m_StartX = x;
    m_StartY = y;

    float lTranslateScaleX = 1.0f * (m_Radius/100.0f);
    float lTranslateScaleY = 1.0f * (m_Radius/100.0f);
    float lTranslateScaleZ = 0.1f * (m_Radius/100.0f);
    float lRotateScaleX = -0.4f;
    float lRotateScaleY = 0.4f;
    float lZoomScale = 0.1f;
    if
      (m_TranslateMode)
    {
      Orientation lOrientation;

      lOrientation = gamma(m_Theta) * beta(m_Phi);

      Position lTranslate;

      lTranslate[0] = lDeltaY * lTranslateScaleY;
      lTranslate[1] = lDeltaX * lTranslateScaleX;
      lTranslate[2] = 0.0f;

      lTranslate = lOrientation * lTranslate;

      m_Center[1] += lTranslate[1];
      m_Center[0] += lTranslate[0];
    }

    if
      (m_ElevationMode)
    {
      m_Center[2] += lDeltaY * lTranslateScaleZ;
    }

    if
      (m_ZoomMode)
    {
      m_Radius += -lDeltaY * lZoomScale - lDeltaY * (m_Radius / 100.0f);
    }

    if
      (m_RotateMode)
    {
      m_Theta += lDeltaX * lRotateScaleX;
      m_Phi += lDeltaY * lRotateScaleY;
    }
    ...
    lPosition = m_Center;
    lPosition[0] -= m_Radius * lOrientation(0,0);
    lPosition[1] -= m_Radius * lOrientation(1,0);
    lPosition[2] -= m_Radius * lOrientation(2,0);

    m_Camera->set_pos(lPosition);

    m_Camera->set_orient(lOrientation);
  }
  ...
  if (m_heli_controller)
  {
    float flt_x = (float) x / m_config.window_x;
    float flt_y = (float) y / m_config.window_y;
    m_heli_controller->mouse_motion(flt_x, flt_y);
  }
}
...
void Sss::do_joystick()
{
  ...
  if (!m_js->notWorking())
  {
    ...
    for (i = 0 ; i < num_ax ; ++i)
    {
      int channel = control_method->get_channel(joysticks[i]);
      if (channel >= 0)
      {
        m_joystick->set_value(
          channel,
...
void Sss::key(unsigned char key, int px, int py)
{
  ...
  if (m_heli_controller)
  {
    if (key == 'K')
    {
      TRACE("Disabling special helicopter controller\n");
      delete m_heli_controller;
      m_heli_controller = 0;
      return;
    }
    else if (true == m_heli_controller->key(key, px, py))
    {
      return;
    }
  }
  ...
  case 'V':
  {
    Vario * l_vario = &m_glider->get_vario();
    if (!l_vario->get_silence())
    {
      if (l_vario->get_location() == Config::EYE)
        l_vario->set_location(Config::GLIDER);
      else if (l_vario->get_location() == Config::GLIDER)
        l_vario->set_silence(true);
    }
    else
    {
      l_vario->set_silence(false);
      l_vario->set_location(Config::EYE);
    }
    break;
  }
...
void Sss::keyUp(unsigned char key, int px, int py)
{
  if (m_heli_controller)
  {
    if (true == m_heli_controller->key_up(key, px, py))
    {
      return;
    }
  }
...
{
  if (Sss::instance()->config().running_as_screensaver && done_first_frame)
    exit(0);
...
void Sss::_keyUp(unsigned char key, int x, int y) 
{
  m_instance->keyUp(key, x, y);
}
