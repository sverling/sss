...
*/
Glider_graphics_3ds::Glider_graphics_3ds(Config_file graphics_config,
                                         Glider & glider)
  :
  max_dist(6), // for now
  m_glider(glider),
  ...
  list_num(glGenLists(2))
{
  ...
  graphics_config.get_value("3ds_file", glider_3ds_file);
  graphics_config.get_value("cull_backface", cull_backface);
...
Glider_graphics_3ds::~Glider_graphics_3ds()
{
  TRACE_METHOD_ONLY(1);
  delete load3ds;
  delete model;
...
void Glider_graphics_3ds::draw_glider(Draw_type draw_type)
{
...
*/
void Glider_graphics_3ds::draw(Draw_type draw_type)
{
  if (
...
void Glider_graphics_3ds::show()
{
  TRACE("Glider_graphics_3ds\n");
}


float Glider_graphics_3ds::calc_bounding_radius() const
{
  float radius = 1.0;
  // Since we know how many objects our model has, go through each of them.
  for(int i = 0; i < model->numOfObjects; i++)
