...
public:
  ...
  void gaussian_filter(float r, int n);
  ...
  //! success. Requires ability to cast to float
...
template<class T>
void Array_2D<T>::gaussian_filter(float r, int n)
{
...
void Array_2D<T>::set_range(T val_min, T val_max)
{
  ...
  float scale = (val_max - val_min) / (orig_max - orig_min);
  float offset = val_min - scale * orig_min;
